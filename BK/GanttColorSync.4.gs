...
// ════════════════════════════════════════════════════════════
//  YEAR SETUP — Interactive year-range initialization
//  v3: Two prompts (from-year / to-year).
//      Week numbering resets to 1 at each new year's week 1.
...
function setupYear() {
  ...
  const r1 = ui.prompt(
    '🗓 אתחל גאנט — שנת התחלה',
    'הכנס שנת התחלה (לדוגמה: 2026):',
    ui.ButtonSet.OK_CANCEL
  );
  if (r1.getSelectedButton() !== ui.Button.OK) return;
  const startYear = parseInt(r1.getResponseText().trim(), 10);
  if (isNaN(startYear) || startYear < 2000 || startYear > 2100) {
    ui.alert('❌ שנה לא תקינה (2000-2100)'); return;
  }

  // ── Prompt 2: to-year ─────────────────────────────────────
  const r2 = ui.prompt(
    '🗓 אתחל גאנט — שנת סיום',
    'הכנס שנת סיום (לדוגמה: ' + startYear + '):',
    ui.ButtonSet.OK_CANCEL
  );
  if (r2.getSelectedButton() !== ui.Button.OK) return;
  const endYear = parseInt(r2.getResponseText().trim(), 10);
  if (isNaN(endYear) || endYear < startYear || endYear > 2100) {
    ui.alert('❌ שנת סיום לא תקינה (חייבת להיות ≥ שנת התחלה)'); return;
  }
  ...
  buildYearGrid_(sheet, startYear, endYear);
...
// ════════════════════════════════════════════════════════════
//  YEAR GRID BUILDER  (v3 — multi-year, week reset per year)
// ════════════════════════════════════════════════════════════
function buildYearGrid_(sheet, startYear, endYear) {
  // ── 1. Compute grid boundaries ────────────────────────────
  //  Grid starts on Sunday of startYear's week 1.
  //  Grid ends on Saturday of endYear's last week.
  const firstThu  = findFirstThursdayOfYear_(startYear);
  const gridStart = new Date(firstThu);
  gridStart.setDate(firstThu.getDate() - firstThu.getDay());   // Sunday

  const dec31End  = new Date(endYear, 11, 31);
  const lastSun   = new Date(dec31End);
  lastSun.setDate(dec31End.getDate() - dec31End.getDay());     // Sunday
  const gridEnd   = new Date(lastSun);
  gridEnd.setDate(lastSun.getDate() + 6);                      // Saturday

  const totalDays  = Math.round((gridEnd - gridStart) / 86400000) + 1;
  const totalWeeks = totalDays / 7;
  const totalCols  = totalDays;

  // ── 2. Pre-compute week-1 Sundays for each year in range ──
  //  When we land on one of these Sundays, weekNum resets to 1.
  const week1Sundays = new Set();
  for (let y = startYear; y <= endYear; y++) {
    const thu = findFirstThursdayOfYear_(y);
    const sun = new Date(thu);
    sun.setDate(thu.getDate() - thu.getDay());
    week1Sundays.add(dateKey_(sun));
  }

  // ── 3. Merge holiday sets for all years in range ──────────
  ...
  for (let i = 0; i < totalCols; i++) {
    ...
    // "other year" = outside the entire requested range
    const isOtherYear = curYear < startYear || curYear > endYear;
    const isHoliday   = holidays.has(dateKey_(cur));

    // Reset week counter at the first Sunday of each year's week 1
    ...
    if (month !== prevMonth) {
      row5vals[i] = new Date(curYear, month, 1);
      prevMonth   = month;
    }
